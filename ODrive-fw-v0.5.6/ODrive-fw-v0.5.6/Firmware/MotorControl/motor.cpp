...
// Debug counters (defined in odrive_main.cpp) — visivel via motor.timstat?
extern "C" volatile uint32_t g_sat_count;
extern "C" volatile uint32_t g_last_sat_adc;
extern "C" volatile uint32_t g_apply_pwm_count;
extern "C" volatile uint32_t g_pwm_update_cb_count;
extern "C" volatile uint32_t g_last_disarm_error;
extern "C" volatile uint32_t g_arm_last_cl_ptr;
extern "C" volatile uint32_t g_last_isr_armed;
extern "C" volatile uint32_t g_last_isr_cl_ptr;
...
extern "C" volatile uint32_t g_arm_call_cnt;
extern "C" volatile uint32_t g_arm_success_cnt;
extern "C" volatile uint32_t g_disarm_call_cnt;
extern "C" volatile uint32_t g_brake_resistor_armed_snapshot;
extern "C" volatile uint32_t g_enable_brake_snapshot;

bool Motor::arm(PhaseControlLaw<3>* control_law) {
    g_arm_call_cnt++;
    odrive_drv_trace(0xF0);
    axis_->mechanical_brake_.release();
    odrive_drv_trace(0xF1);
    ...
    CRITICAL_SECTION() {
        ...
        odrive_drv_trace(0xF2);

        extern bool brake_resistor_armed;
        // Snapshot dos valores no momento exato do check
        g_enable_brake_snapshot = odrv.config_.enable_brake_resistor ? 1 : 0;
        g_brake_resistor_armed_snapshot = brake_resistor_armed ? 1 : 0;
        if (!odrv.config_.enable_brake_resistor || brake_resistor_armed) {
            odrive_drv_trace(0xF3);
            armed_state_ = 1;
            is_armed_ = true;
            // Compiler barrier + data memory barrier — garante que ISR veja os
            // writes a control_law_, armed_state_, is_armed_ antes de qualquer
            // pwm_update_cb rodar a partir de agora.
            __asm__ volatile("" ::: "memory");
            __DMB();
            g_arm_success_cnt++;
            g_arm_last_cl_ptr = (uint32_t)control_law_;
        } else {
            odrive_drv_trace(0xF4);
            error_ |= Motor::ERROR_BRAKE_RESISTOR_DISARMED;
        }
    }
    odrive_drv_trace(0xF5);
    return true;
}
 ...
 * arm() is called.
 */
extern "C" volatile uint32_t g_disarm_caller;
extern "C" volatile uint32_t g_disarm_callers[8];
extern "C" volatile uint8_t g_disarm_idx;

bool Motor::disarm(bool* p_was_armed) {
    bool was_armed;
    g_disarm_call_cnt++;
    g_disarm_caller = (uint32_t)__builtin_return_address(0);  // quem chamou
    if (g_disarm_idx < 8) {
        g_disarm_callers[g_disarm_idx++] = g_disarm_caller;
    }
    g_last_disarm_error = (uint32_t)error_;
    odrive_drv_trace(0xE9);  // disarm called
    CRITICAL_SECTION() {
        was_armed = is_armed_;
        if (is_armed_) {
            odrive_drv_trace(0xEA);  // actually was armed
...
bool Motor::measure_phase_resistance(float test_current, float max_voltage) {
    ...
    // v0.5.1 parity: UNBALANCED_PHASES check does not exist in v0.5.1 and trips
    // on MKS Mini hardware due to residual current-sense DC offset. Since the
    // resistance value itself (via control_law.get_resistance()) is already
    // validated above, the balance check is redundant for our use case.
...
void Motor::current_meas_cb(uint32_t timestamp, std::optional<Iph_ABC_t> current) {
    ...
    // dc_calib_valid = garante que DC offset do current sense convergiu.
    // v0.5.1 parity: v0.5.1 does not have this gate at all — offsets are applied
    // from cycle 1 and the DC filter converges in background. v0.5.6 default
    // (7.5*tau ≈ 1.5 s) blocks current_meas for half of the calibration window,
    // which prevents R/L convergence on MKS Mini. Drop to 1*tau (≈200 ms) which
    // is sufficient for the single-pole filter to settle while leaving >=2.5 s
    // of the 3 s calibration window with valid current feedback.
    bool dc_calib_valid = (dc_calib_running_since_ >= config_.dc_calib_tau * 1.0f);
...
extern "C" volatile uint32_t g_pwm_is_armed_cnt;
extern "C" volatile uint32_t g_pwm_status_none_cnt;
extern "C" volatile uint32_t g_pwm_status_init_cnt;
extern "C" volatile uint32_t g_pwm_status_other_cnt;
extern "C" volatile uint32_t g_pwm_cl_null_cnt;
extern "C" volatile uint32_t g_pwm_last_status;

void Motor::pwm_update_cb(uint32_t output_timestamp) {
    TaskTimerContext tmr{axis_->task_times_.pwm_update};
    n_evt_pwm_update_++;
    g_pwm_update_cb_count++;
    ...
    g_pwm_last_status = (uint32_t)control_law_status;
    g_last_isr_armed = is_armed_ ? 1 : 0;
    g_last_isr_cl_ptr = (uint32_t)control_law_;
    if (is_armed_) g_pwm_is_armed_cnt++;
    if (control_law_status == ERROR_NONE) g_pwm_status_none_cnt++;
    else if (control_law_status == ERROR_CONTROLLER_INITIALIZING) g_pwm_status_init_cnt++;
    else g_pwm_status_other_cnt++;
    ...
    // Only check I_bus hard limits when armed, and NOT during motor calibration.
    // v0.5.1 parity: v0.5.1 has no ISR-side I_bus check at all; the check was
    // added in v0.5.6 and fires on pre-arm/idle NaN values on MKS Mini. Also,
    // during R/L measurement the current_meas_->i_bus computation goes through
    // the measurement control law which reports transient values the hard
    // limit can trip on. Skip during AXIS_STATE_MOTOR_CALIBRATION.
    if (is_armed_
        && axis_->current_state_ != Axis::AXIS_STATE_MOTOR_CALIBRATION
        && (*i_bus < config_.I_bus_hard_min || *i_bus > config_.I_bus_hard_max)) {
